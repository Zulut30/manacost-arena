...
- Добавлен приём личных данных трекера IceCrow: `POST
  /api/v1/tracker/events/batch` принимает до 50 идемпотентных событий
  (ranked-, Arena- и Battlegrounds-матчи, забеги и пики Арены, снимок
  коллекции) по bearer-токену со scope `tracker.write`, а
  `GET /api/v1/tracker/profile/summary`, `/matches` и `/arena/runs` отдают
  сводку и историю только владельцу через `tracker.read` или сессию браузера.
  Сырые логи и имена игроков не передаются; точная колода соперника
  раскрывается только по HMAC-ключу игры (`TRACKER_JOIN_SECRET`) из его
  собственной точной отправки. Контракт:
  `docs/specs/tracker-profile-ingestion-v1.md`.
